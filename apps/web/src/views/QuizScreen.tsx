            ...
            <div className="flex flex-col justify-center gap-2 border-t border-[#30455f] pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                <SpeakButton
                  ...
                  iconOnly
                  onClick={() =>
                    void speak(currentQuestion.prompt.replace('___', '……'))
                  }
                />
                <SpeakButton
                  label={copy.slowLabel}
                  icon="slow"
                  iconOnly
                  onClick={() =>
                    void speak(currentQuestion.prompt.replace('___', '……'), {
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 gap-2">
                      <SpeakButton
                        ...
                        iconOnly
                        className="px-2 py-2"
                        onClick={() => void speak(option.hanzi)}
                      />
                      <SpeakButton
                        label={`${copy.slowLabel}: ${option.hanzi}`}
                        icon="slow"
                        iconOnly
                        className="px-2 py-2"
                        onClick={() =>
                          void speak(option.hanzi, {
                            rate: slowRate
                          })
                        }
                      />
                ...
                <div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <SpeakButton
                      ...
                      icon="speak"
                      iconOnly
                      onClick={() => void speak(correctClassifier.hanzi)}
                    />
                    <SpeakButton
                      label={`${copy.slowLabel}: ${correctClassifier.hanzi}`}
                      icon="slow"
                      iconOnly
                      onClick={() =>
                        void speak(correctClassifier.hanzi, {
                          rate: slowRate
                        })
                      }
                    />
                ...
                {correctClassifier.examples.map((example) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-2 text-[11px] uppercase tracking-[0.18em] text-[#8c2f22]">
                        {example.pinyin.surface}
                      </p>
                      {example.pinyin.bopomofo ? (
                        <p className="mt-1 text-[11px] uppercase tracking-[0.16em] text-[#5b6f84]">
                          {example.pinyin.bopomofo}
                        </p>
                      ) : null}
                    ...
                    <div className="flex gap-2">
                      <SpeakButton
                        ...
                        icon="speak"
                        iconOnly
                        onClick={() => void speak(example.hanzi)}
                      />
                      <SpeakButton
                        label={`${copy.slowLabel}: ${example.hanzi}`}
                        icon="slow"
                        iconOnly
                        onClick={() =>
                          void speak(example.hanzi, {
                            rate: slowRate
                          })
                        }
                      />
